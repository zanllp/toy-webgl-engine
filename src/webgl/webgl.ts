...
import { array2Vec3, Box, createProgramInfo, createSetValueFn, degToRad, modifyWindow, printMat, resize, Scene, createKeyListener, Sphere, Mesh, Point, Assembly, GL } from './tool';

import color from 'color-name';
import t from 'towa.jpg';
...
const createModel = (gl: WebGLRenderingContext) => {
    ...
    for (let i = 0; i < 1; i++) {
        const s = new Sphere({ radius: 400, color: 'rand' });
        s.setModelMat(x => {
            mat4.translate(x, x, [0, 800 + i * 400, 0]);
        });
        spheres.push(s);
    }
    ...
    const tsp = spheres[0].childArray[3];
...
export class App extends GL<ReturnType<typeof createInfo>, typeof state.value> {
    constructor(gl: WebGLRenderingContext) {
        super();
        this.gl = gl;
        const setV = this.setState.bind(this);
        createKeyListener({
    ...
    public info = createInfo(this.gl);
